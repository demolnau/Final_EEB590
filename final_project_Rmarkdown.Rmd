...
# Y<-data.frame(lapply(Y,function(x) as.numeric(levels(x))[x]))
?lm.rrpp
mydat<-rrpp.data.frame("Y"=imputed[,7:ncol(imputed)],"Group_2"= as.factor(imputed$Group_2), "Habitat"=as.factor(imputed$Habitat), "Country" = as.factor(imputed$Country))

#model2.rrpp <- lm.rrpp(mydat$Y ~ mydat$Group_2 * mydat$Habitat * mydat$Country , print.progress = FALSE)
#anova(model2.rrpp)
